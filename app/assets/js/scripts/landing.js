    ...
    sysAEx.on('message', (m) => {
        ...
        if (m.context === 'validateJava') {
            if (m.result == null) {
                ...
                setOverlayContent(
                    'Javaが見つかりませんでした',
                    'サーバーに参加するには、64ビット版のJava 17が必要です。自動的にダウンロードしてインストールしますか？',
                    '自動でインストール',
                    '自分でいすんとーるする'
                )
                setOverlayHandler(() => {
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Javaが必要です',
                            '起動には64ビット版のJava 17が必要ですが、インストールされていません。',
                            'わかった！',
                            'はい...'
                        )
                        setOverlayHandler(() => {
            ...
            if (m.result === true) {
                ...
                setOverlayContent(
                    'ｳｰﾝ、Javaのダウンロードに失敗！ｗ',
                    'とても不幸なことですが、Javaのダウンロードに失敗しました。',
                    'わかった'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
...
function dlAsync(login = true) {
    ...
    aEx.on('close', (code, signal) => {
        if (code !== 0) {
            ...
            showLaunchFailure('ありゃ...', 'ゲームの起動に失敗しました。')
        }
    })
    ...
    aEx.on('message', (m) => {
        ...
        if (m.context === 'validate') {
            ...
            if (m.result.forgeData == null || m.result.versionData == null) {
                ...
                showLaunchFailure('あちゃー...', 'ゲームの起動に失敗しました。')
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    let content = el.find('content').text().replaceAll('\\n', '<br>')
